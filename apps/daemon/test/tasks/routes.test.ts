...
describe("GET /projects/{project}/tasks", () => {
  /**
   * Three tasks and one file the index cannot place. `SAGA-1` is blocked by a
   * task that is final and so is not blocked; `SAGA-3` names a blocker no file
  ...
  async function tree(): Promise<string> {
    ...
    await plant(join(tasks, "SAGA-1.md"), entryText("SAGA-1", "To Do", [
      "priority: high",
      "labels: [dev, ui]",
      "parent: SAGA-9",
      "step: dev:review",
    ...
    await plant(join(tasks, "SAGA-2.md"), entryText("SAGA-2", "Done", []));
    await plant(join(tasks, "SAGA-3.md"), entryText("SAGA-3", "In Progress", ["blocked_by: [SAGA-77]"]));
    await plant(join(tasks, "SAGA-4.md"), entryText("SAGA-5", "To Do", []));
    return root;
  }

  async function listed(server: TestServer, search: string): Promise<string[]> {
    const response = await send(server, "GET", `/projects/SAGA/tasks${search}`);
  ...
  it("answers with every entry of the project and the files it could not place", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks");
    ...
    expect(data.excluded).toEqual([
      { path: taskFile(root, "SAGA", "SAGA-4"), code: "task-file-misnamed", message: expect.any(String) as string },
    ]);
    expect(diagnostics).toEqual([
      {
        code: "blocked-by-unresolved",
        message: expect.stringContaining("SAGA-77") as string,
        path: taskFile(root, "SAGA", "SAGA-3"),
      },
    ]);
  });
  ...
  it("flags a blocked entry that another filter kept, and reports its unresolved blocker", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks?status=In+Progress");

    const { data, diagnostics } = await success<TaskList>(response);
    expect(data.entries.map(({ id, blocked }) => ({ id, blocked }))).toEqual([{ id: "SAGA-3", blocked: true }]);
    expect(diagnostics).toEqual([
      {
        code: "blocked-by-unresolved",
        message: expect.stringContaining("SAGA-77") as string,
  ...
  it.each([
    ["status", "?status=to+do", ["SAGA-1"]],
    ["priority", "?priority=HIGH", ["SAGA-1"]],
    ["label", "?label=dev&label=ui", ["SAGA-1"]],
    ["parent", "?parent=SAGA-9", ["SAGA-1"]],
    ["step", "?step=dev:review", ["SAGA-1"]],
  ...
  it("applies two filters together", async () => {
    ...
    await expect(listed(server, "?label=dev&status=To+Do")).resolves.toEqual(["SAGA-1"]);
  });

  it("keeps the blocked entries alone under blocked=true", async () => {
    const server = await serving(await tree(), taskRoutes);

    await expect(listed(server, "?blocked=true")).resolves.toEqual(["SAGA-3"]);
  });

  it("keeps the unblocked entries alone under blocked=false", async () => {
    const server = await serving(await tree(), taskRoutes);

    await expect(listed(server, "?blocked=false")).resolves.toEqual(["SAGA-1", "SAGA-2"]);
  });
  ...
  it("reports a blocker that names no task once, whether or not the filter kept its task", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks?blocked=false");

    const { data, diagnostics } = await success<TaskList>(response);
    expect(data.entries.map((entry) => entry.id)).not.toContain("SAGA-3");
    expect(diagnostics).toEqual([
      {
        code: "blocked-by-unresolved",
        message: expect.stringContaining("SAGA-77") as string,
  ...
  it("carries the findings of the configuration file for a listing that states no filter", async () => {
    ...
    const { diagnostics } = await success<TaskList>(await send(server, "GET", "/projects/SAGA/tasks"));
  ...
  it("refuses a listing that states no filter over an invalid configuration", async () => {
    const root = await projectsRoot("SAGA");
    await plant(projectConfig(root, "SAGA"), "statuses: [To Do, Done]\nfinal_statuses: [Closed]\n");
    await plant(taskFile(root, "SAGA", "SAGA-1"), taskText("SAGA-1"));
    const server = await serving(root, taskRoutes);
...
describe("GET /projects/{project}/tasks/{id}", () => {
  it("answers with the whole task, its comments among them", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-1");

    expect(response.status).toBe(200);
    const { data } = await success<{ frontmatter: { id: string }; body: string; comments: { body: string }[] }>(
      response,
    );
    expect(data.frontmatter.id).toBe("SAGA-1");
  ...
  it("leaves the comments key out altogether under comments=false", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-1?comments=false");
  ...
  it("answers with the comments under comments=true, which asks for the whole task", async () => {
    ...
    const { data } = await success<{ comments: unknown[] }>(
      await send(server, "GET", "/projects/SAGA/tasks/SAGA-1?comments=true"),
    );
  ...
  it("refuses a comments option that is neither true nor false", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-1?comments=maybe");
  ...
  it("forwards a store refusal of a task that does not exist", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-9");
  ...
  it("forwards a parse refusal of a file that will not read back", async () => {
    const root = await projectsRoot("SAGA");
    await plant(taskFile(root, "SAGA", "SAGA-1"), "no frontmatter here\n");
    const server = await serving(root, taskRoutes);
...
describe("GET /projects/{project}/tasks/{id}/text", () => {
  ...
  async function planted(text = taskWithComments("SAGA-1")): Promise<{ server: TestServer; text: string }> {
    const root = await projectsRoot("SAGA");
    await plant(taskFile(root, "SAGA", "SAGA-1"), text);
    return { server: await serving(root, taskRoutes), text };
  }

  async function textOf(server: TestServer, search: string): Promise<TaskText> {
    const response = await send(server, "GET", `/projects/SAGA/tasks/SAGA-1/text${search}`);
    expect(response.status).toBe(200);
  ...
  it("refuses a comment id the file carries no comment under", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-1/text?comment=9");
  ...
  it("forwards a store refusal of a task that does not exist", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-9/text");
  ...
  it("refuses the two selections together", async () => {
    ...
    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-1/text?comment=2&collapsed=false");
  ...
  it("carries the findings of the file it read on the envelope", async () => {
    const { server } = await planted(taskWithComments("SAGA-1").replace("next_comment_id: 3", "next_comment_id: 2"));

    const response = await send(server, "GET", "/projects/SAGA/tasks/SAGA-1/text");
...
describe("the write routes over a task", () => {
  ...
  beforeEach(async () => {
    ...
    await plant(taskFile(root, "SAGA", "SAGA-1"), entryText("SAGA-1", "To Do", ["priority: high"]));
    server = await serving(root, taskRoutes);
  });

  it("creates a task and answers with the write receipt", async () => {
    const response = await send(server, "POST", "/projects/SAGA/tasks", { title: "Write it", body: "text" });

    expect(response.status).toBe(200);
    const { data } = await success<WriteResult>(response);
    expect(data).toEqual({ id: "SAGA-2", status: "Backlog" });
    await expect(readFile(taskFile(root, "SAGA", "SAGA-2"), "utf8")).resolves.toContain("title: Write it");
  });

  it("reads an absent body as a change that sets nothing, which the engine then refuses", async () => {
    const response = await send(server, "POST", "/projects/SAGA/tasks");
  ...
  it("updates a task and answers with the write receipt", async () => {
    const response = await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { status: "Done" });

    expect(response.status).toBe(200);
    await expect(success<WriteResult>(response)).resolves.toMatchObject({ data: { id: "SAGA-1", status: "Done" } });
  });

  it("clears a field named with null", async () => {
    const response = await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { priority: null });

    expect(response.status).toBe(200);
    await expect(readFile(taskFile(root, "SAGA", "SAGA-1"), "utf8")).resolves.not.toContain("priority");
  });
  ...
  it("leaves a field the change does not name alone", async () => {
    ...
    expect(response.status).toBe(200);
    await expect(readFile(taskFile(root, "SAGA", "SAGA-1"), "utf8")).resolves.toContain("priority: high");
  });

  it("deletes a task and answers with the write receipt", async () => {
    const response = await send(server, "DELETE", "/projects/SAGA/tasks/SAGA-1");

    expect(response.status).toBe(200);
    await expect(success<WriteResult>(response)).resolves.toEqual({ data: { id: "SAGA-1" }, diagnostics: [] });
    await expect(readFile(taskFile(root, "SAGA", "SAGA-1"), "utf8")).rejects.toThrow();
  });

  it("takes the deleted id out of a task that named it, which the blocked filter then lists no more", async () => {
    await plant(taskFile(root, "SAGA", "SAGA-2"), entryText("SAGA-2", "To Do", ["blocked_by: [SAGA-1]"]));
    ...
    const listed = await send(server, "GET", "/projects/SAGA/tasks?blocked=true");
    await expect(success<TaskList>(listed)).resolves.toMatchObject({ data: { entries: [] } });
    const all = await success<TaskList>(await send(server, "GET", "/projects/SAGA/tasks"));
    expect(all.data.entries.map(({ id, blocked }) => ({ id, blocked }))).toEqual([{ id: "SAGA-2", blocked: false }]);
  });

  it.each([
    ["POST", "/projects/SAGA/tasks", { title: "Blocked" }],
    ["PATCH", "/projects/SAGA/tasks/SAGA-1", {}],
    ...
    const response = await send(server, method, path, { ...fields, blocked_by: [...unknown, "SAGA-2"] });
  ...
  it("forwards a store refusal of a field the engine owns", async () => {
    const response = await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { id: "SAGA-9" });
  ...
  it.each([
    ...
    const response = await send(server, "POST", "/projects/SAGA/tasks", body);
  ...
  it("refuses any query key at all on a write route", async () => {
    const response = await send(server, "DELETE", "/projects/SAGA/tasks/SAGA-1?comments=false");
...
describe("the step of a task, set through the patch that writes it", () => {
  ...
  beforeEach(async () => {
    root = await projectsRoot("SAGA");
    await plant(projectConfig(root, "SAGA"), "workflows: [dev]\n");
    await plantSteps(root, "dev", "dev:research", "dev:implement");
    await plant(taskFile(root, "SAGA", "SAGA-1"), entryText("SAGA-1", "To Do", ["workflow: dev"]));
    server = await serving(root, taskRoutes);
  });

  it("writes a step the task's workflow declares", async () => {
    const response = await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { step: "dev:research" });

    expect(response.status).toBe(200);
    await expect(success<WriteResult>(response)).resolves.toEqual({ data: { id: "SAGA-1" }, diagnostics: [] });
    await expect(readFile(taskFile(root, "SAGA", "SAGA-1"), "utf8")).resolves.toContain("step: dev:research");
  });

  it("clears the step named with null", async () => {
    await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { step: "dev:research" });

    const response = await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { step: null });

    expect(response.status).toBe(200);
    await expect(readFile(taskFile(root, "SAGA", "SAGA-1"), "utf8")).resolves.not.toContain("step:");
  });

  it("refuses a step the task's workflow does not declare", async () => {
    const response = await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { step: "dev:release" });
  ...
  it("refuses a workflow the project does not declare, along with the step of it", async () => {
    ...
    const response = await send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", {
      workflow: "review",
      step: "review:read",
    });
...
describe("two writes to one project that arrive at once", () => {
  ...
  it("keeps both of two updates of one task", async () => {
    const responses = await Promise.all([
      send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { title: "Renamed" }),
      send(server, "PATCH", "/projects/SAGA/tasks/SAGA-1", { status: "Done" }),
    ]);
  ...
  it("runs a delete after the patch of a task it takes a reference out of, and keeps both changes", async () => {
    ...
    const patching = send(held, "PATCH", "/projects/SAGA/tasks/SAGA-2", { title: "Renamed" });
    ...
    const text = await readFile(taskFile(root, "SAGA", "SAGA-2"), "utf8");
    expect(text).toContain("title: Renamed");
  ...
  it("runs a delete after a patch that states the deleted task as a blocker, and takes the blocker out", async () => {
    ...
    const patching = send(held, "PATCH", "/projects/SAGA/tasks/SAGA-2", { blocked_by: ["SAGA-1"] });
    await until(() => reached.includes("patch"), "the patch took its turn");
    const deleted = await send(held, "DELETE", "/projects/SAGA/tasks/SAGA-1");
  ...
  it("runs a delete after a create that states the deleted task as a blocker, and takes the blocker out", async () => {
    ...
    const creating = send(held, "POST", "/projects/SAGA/tasks", { title: "Blocked", blocked_by: ["SAGA-1"] });
    ...
    await expect(readFile(taskFile(root, "SAGA", created.data.id), "utf8")).resolves.not.toContain("blocked_by");
  });

  it("gives each of eight creates its own id and its own file", async () => {
    const responses = await Promise.all(
      Array.from({ length: 8 }, (_, n) => send(server, "POST", "/projects/SAGA/tasks", { title: `Task ${n}` })),
    );
    ...
    for (const id of ids) {
      await expect(readFile(taskFile(root, "SAGA", id), "utf8")).resolves.toContain("title: Task");
    }
  });
});
