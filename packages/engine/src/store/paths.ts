...
/**
 * True when a name a caller stated resolves to an entry of the kind `holds`
 * accepts. The link is followed: the entry is the caller's own, so a link to it
 ...
 * put it right.
 */
async function namesEntry(path: string, holds: (stats: Stats) => boolean): Promise<boolean> {
  try {
    return holds(await stat(path));
  } catch (error) {
...
/**
 * The absolute path of an entry a caller stated, which must be there. The
 ...
 *
 * `subject` opens each refusal, so one rule serves every entry a caller names,
 * and each is refused in its own words.
 */
async function checkedEntryPath(
  stated: string,
  subject: string,
  holds: (stats: Stats) => boolean,
  kind: string,
): Promise<string> {
  ...
  if (!(await namesEntry(resolved, holds))) fail("path-invalid", `this path names no ${kind}`, resolved);
  return resolved;
}

/** The same, for a directory. */
export async function checkedDirectoryPath(stated: string, subject: string): Promise<string> {
  return checkedEntryPath(stated, subject, (stats) => stats.isDirectory(), "directory");
}

/** The same, for a regular file. */
export async function checkedFilePath(stated: string, subject: string): Promise<string> {
  return checkedEntryPath(stated, subject, (stats) => stats.isFile(), "regular file");
}
