...
describe("the entries of the index", () => {
  it("holds one entry per task file, ordered by file number ascending", async () => {
    ...
    for (const id of ["SAGA-10", "SAGA-2", "SAGA-1"]) await plant(taskFile(root, id), taskText(id));

    expect(ids(await built(root))).toEqual(["SAGA-1", "SAGA-2", "SAGA-10"]);
  });

  it("carries the id, the path and the frontmatter of the file", async () => {
    const root = await tempRoot();
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1", "priority: high\n"));

    const [entry] = (await built(root)).query().entries;

    expect(entry).toEqual({
      id: "SAGA-1",
      path: taskFile(root, "SAGA-1"),
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- an asymmetric matcher is typed `any`
      frontmatter: expect.objectContaining({ id: "SAGA-1", title: "Planted", priority: "high" }),
    });
  });
  ...
  it("reads a file the whole parse refuses under the frontmatter", async () => {
    ...
    expect(ids(await built(root))).toEqual(["SAGA-1"]);
  });

  it("hands out an entry no caller can change", async () => {
    const root = await tempRoot();
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1", "labels: [one]\n"));
...
describe("the files the index excludes", () => {
  it("excludes a file that does not parse and reports it", async () => {
    ...
    expect(result.excluded).toEqual([
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- an asymmetric matcher is typed `any`
      { path: taskFile(root, "SAGA-1"), code: "task-file-unreadable", message: expect.stringContaining("---") },
    ]);
    expect(seen.codes()).toEqual(["task-file-unreadable"]);
    expect(seen.seen[0]?.path).toBe(taskFile(root, "SAGA-1"));
  });

  it("carries the line of the fault to the listener", async () => {
    const root = await tempRoot();
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1").replace("title: Planted", "title: [one]"));
    const seen = listener();
  ...
  it("excludes a file whose id names no task of this project", async () => {
    const root = await tempRoot();
    await plant(taskFile(root, "SAGA-1"), taskText("OTHER-1"));
  ...
  // either way.
  it.each([
    ["by number", ["SAGA-1", "SAGA-2", "SAGA-10"]],
    ["in reverse", ["SAGA-10", "SAGA-2", "SAGA-1"]],
    ...
    expect(cache.query().excluded.map((file) => file.path)).toEqual([
      taskFile(root, "SAGA-1"),
      taskFile(root, "SAGA-10"),
      taskFile(root, "SAGA-2"),
    ]);
  });

  it("states why a file cannot be read without naming the class that refused it", async () => {
    const root = await tempRoot();
    await plant(taskFile(root, "SAGA-1"), "---\nid: SAGA-1\n");

    const [excluded] = (await built(root)).query().excluded;

    expect(excluded?.message).not.toContain("TaskParseError");
    expect(excluded?.message).toContain(taskFile(root, "SAGA-1"));
  ...
  it("cuts an id the file supplied down to a length a message can carry", async () => {
    ...
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1").replace("id: SAGA-1", `id: ${long}`));
  ...
  it("cuts a reason as long as a file cared to make it", async () => {
    ...
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1", `? "${"x".repeat(2000)}"\n: &loop [*loop]\n`));
    const seen = listener();
  ...
  it("keeps out of a message every character a file supplied that would end a line", async () => {
    ...
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1", '? "one\\ntwo\\u2028three\\u2029four"\n: &loop [*loop]\n'));
  ...
  it("cuts a value on a character rather than in the middle of one", async () => {
    ...
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1").replace("id: SAGA-1", `id: "${id}"`));
  ...
  it("excludes a file whose frontmatter stands past the cap, naming the bound it read to", async () => {
    ...
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1", `note: ${"x".repeat(FRONTMATTER_CAP)}\n`));
    const seen = listener();
  ...
  it("excludes a file it may not read", async () => {
    const root = await tempRoot();
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1"));
    await chmod(taskFile(root, "SAGA-1"), 0o000);
    onTestFinished(() => chmod(taskFile(root, "SAGA-1"), 0o600));
    const seen = listener();
  ...
  it("keeps a temp file and a foreign markdown file out of the excluded set", async () => {
    const root = await tempRoot();
    await plant(join(tasksDir(root), ".SAGA-1.md.abcd.tmp"), "half a write");
...
describe("what the index reports", () => {
  ...
  it("reports a temp file that came back after it was gone", async () => {
    const root = await tempRoot();
    const temp = join(tasksDir(root), ".SAGA-1.md.abcd.tmp");
  ...
  it("keeps the map and the caller unharmed when a listener throws", async () => {
    ...
    expect(ids(cache)).toEqual(["SAGA-2"]);
...
describe("applying one path", () => {
  it("adds the file that appeared", async () => {
    ...
    await cache.apply(taskEntry(root, "SAGA-1"));

    expect(ids(cache)).toEqual(["SAGA-1"]);
  });
  ...
  it("replaces the entry of a file that changed", async () => {
    ...
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1").replace("Planted", "Edited"));
    await cache.apply(taskEntry(root, "SAGA-1"));
  ...
  it("lands no entry it read through a project directory a symbolic link replaced", async () => {
    ...
    await plant(join(root, "outside", "tasks", "SAGA-2.md"), taskText("SAGA-2"));
    await rm(projectDir(root), { recursive: true });
    await symlink(join(root, "outside"), projectDir(root));
    await cache.apply(taskEntry(root, "SAGA-2"));
  ...
  it("lands nothing it read while a symbolic link replaced the project directory", async () => {
    const root = await tempRoot();
    await plant(taskFile(root, "SAGA-1"), taskText("SAGA-1"));
    await plant(join(root, "outside", "tasks", "SAGA-1.md"), taskText("SAGA-1"));
    const seen = listener();
    ...
    const applying = cache.apply(taskEntry(root, "SAGA-1"));
    await reading;
    await rm(projectDir(root), { recursive: true });
    await symlink(join(root, "outside"), projectDir(root));
    release();
  ...
  it("lets the last request decide when two applies overlap", async () => {
    ...
    const first = cache.apply(taskEntry(root, "SAGA-1"));
    await writeFile(taskFile(root, "SAGA-1"), taskText("SAGA-1").replace("Planted", "Last"), "utf8");
    const second = cache.apply(taskEntry(root, "SAGA-1"));
...
describe("reconciling the whole directory", () => {
  it("reads several files at a time for a scan a caller waits on", async () => {
    const root = await tempRoot();
    for (const id of ["SAGA-1", "SAGA-2", "SAGA-3"]) await plant(taskFile(root, id), taskText(id));
    const { cache, peak } = counting(root);

    await cache.reconcile();

    expect(peak()).toBeGreaterThan(1);
    expect(ids(cache)).toEqual(["SAGA-1", "SAGA-2", "SAGA-3"]);
  });

  it("holds no more of a scan in flight than it reads files at a time", async () => {
    const root = await tempRoot();
    const ids = ["SAGA-1", "SAGA-2", "SAGA-3", "SAGA-4", "SAGA-5"];
  ...
  it("reports a tasks name that became a fault after an empty project was opened", async () => {
    ...
    await plant(join(root, "outside", "SAGA-1.md"), taskText("SAGA-1"));
